...
pipe table. ids monotonic (never reused).
...
`§<section>.<item>` = section.item. `§V.2` = invariants section, item 2.
...
> Fixed a bug where token expiry comparison used strict less-than instead of less-than-or-equal, causing tokens to be rejected at the exact moment of their expiry timestamp.
